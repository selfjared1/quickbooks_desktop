...
class TestCopyFromParent(unittest.TestCase):
    ...
    #     element = et.fromstring(self.journal_xml)
    #     journal_entry = JournalEntry.from_xml(element)
    #     jounal_entry_add_wo_ids = JournalEntry.Add.create_add_or_mod_from_parent(journal_entry, add_or_mod='Add',
    #                                                                              keep_ids=False)
    ...
    #     element = et.fromstring(self.journal_xml)
    #     journal_entry = JournalEntry.from_xml(element)
    #     jounal_entry_mod_wo_ids = JournalEntry.Mod.create_add_or_mod_from_parent(journal_entry, add_or_mod='Mod',
    #                                                                              keep_ids=False)
    ...
    def test__create_inventory_adjustment_line_add_01(self):
        parent = InventoryAdjustmentLine(
            Add=InventoryAdjustmentLineAdd,
            txn_line_id='FD588-1686179311',
            item_ref=ItemRef(
                list_id='8000640D-1629926790',
                full_name='Miscellaneous:KAWIG4-10728'
            ),
            serial_number=None,
            serial_number_added_or_removed=None,
            lot_number=None,
            inventory_site_location_ref=None,
            quantity_difference=1.0,
            value_difference=Decimal('0.00')
        )
        instance = InventoryAdjustmentLineAdd()
        instance._create_inventory_adjustment_line_add(parent)
        self.assertIsNotNone(instance)
        self.assertIsNotNone(instance.quantity_adjustment)
        self.assertEqual(instance.item_ref.full_name, parent.item_ref.full_name)
        self.assertEqual(instance.quantity_adjustment.quantity_difference, 1.0)
        self.assertIsNone(instance.value_adjustment)
        self.assertIsNone(instance.serial_number_adjustment)
        self.assertIsNone(instance.lot_number_adjustment)

        parent.value_difference = Decimal('1.00')
        instance = InventoryAdjustmentLineAdd()
        instance._create_inventory_adjustment_line_add(parent)

        self.assertIsNotNone(instance)
        self.assertIsNone(instance.quantity_adjustment)
        self.assertIsNotNone(instance.value_adjustment)
        self.assertEqual(instance.value_adjustment.quantity_difference, 1.0)
        self.assertEqual(instance.value_adjustment.value_difference, Decimal('1.00'))
        self.assertIsNone(instance.serial_number_adjustment)
        self.assertIsNone(instance.lot_number_adjustment)

        parent.quantity_difference = None
        instance = InventoryAdjustmentLineAdd()
        instance._create_inventory_adjustment_line_add(parent)
        self.assertIsNone(instance.quantity_adjustment)
        self.assertEqual(instance.value_adjustment.quantity_difference, 0)
        self.assertEqual(instance.value_adjustment.value_difference, Decimal('1.00'))
        self.assertIsNone(instance.serial_number_adjustment)
        self.assertIsNone(instance.lot_number_adjustment)


        #todo: Test serial and lot changes